...
import com.example.cyclopath.items.Marker;
import com.example.cyclopath.items.Tile;
import com.example.cyclopath.util.Dual_Rect;
   ...
   */
   ...
   public void handleMouseMove(MotionEvent event) {
      if (this.dragging || this.drag_start_valid) {
         ...
         if ( (float) Math.sqrt(x * x + y * y) < Constants.MIN_DRAG_DISTANCE) {
            return;
         }
         this.dragging = true;
         has_panned = true;
         ...
         this.new_distance = (float) Math.sqrt(x * x + y * y);
   ...
   public void handlePointerDown(MotionEvent event) {
      ...
      float x = event.getX(0) - event.getX(1);
      float y = event.getY(0) - event.getY(1);
      this.old_distance = (float) Math.sqrt(x * x + y * y);
